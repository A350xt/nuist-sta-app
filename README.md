...
- [x] 学习页学生卡：当前学期、周次与学期进度
- [x] 双创学分：总分、成绩评定与已认定项目明细（创新创业学分认定系统）
- [x] 劳动积分：官方核算总分与四分项，详情页与各分项实时统计对照（劳动教育平台）
- [ ] 分院帽
- [ ] 成绩查询/通知
...
- [ ] 小公交位置实时显示
- [x] 实时电费/历史电费
- [x] 空教室显示/更易用的筛选（教务 kxjas：按楼 / 日期查，时段 / 楼层 / 类型本地筛选，逐节占用详情）
...
└── mini_apps/                 # 所有小程序，每个一个目录
    ...
    ├── electricity/           # 宿舍电费：首页常驻卡片 + 详情页（icard）
    ├── free_classroom/        # 空教室：按楼 / 日期查教务占用，本地按时段 / 楼层 / 类型筛选（jwxt kxjas）
    ├── student_info/          # 学习页学生卡：身份 + 当前学期（信息门户）
    ├── innovation_credit/     # 双创学分：学习页卡片 + 详情页（cxcyjy → cxxf）
    ├── labor_score/           # 劳动积分：学习页卡片 + 详情页（labor）
